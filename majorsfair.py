...
all_lists = creative_list + leadership_list + life_list + \
...
for row in responses.iterrows():
    ...
    names_to_major_dict[str(row[1][1])] = find_best_match(majors)
    names_to_minor_dict[str(row[1][1])] = find_best_match(minors)
    names_to_cat_dict[str(row[1][1])] = find_best_match(certificates)
...
for key in sorted(names_to_major_dict.keys()):
    if key in names_to_zoom_dict.keys():
        ...
        organized_dict[key + ' - Majors'] = [names_to_zoom_dict[key], *sorted(names_to_major_dict[key].values())]
        organized_dict[key + ' - Minors'] = ['', *sorted(names_to_minor_dict[key].values())]
        organized_dict[key + ' - Certificates'] = ['', *sorted(names_to_cat_dict[key].values())]
